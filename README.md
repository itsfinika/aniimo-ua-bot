...
All three drafting prompts carry the same terminology rules, so the channel reads consistently. The game is **Aniimo** and is never translated or transliterated; the creatures are «аніімо» in Ukrainian, lowercase and indeclinable («нове аніімо», «86 аніімо»); players are Pathfinders → «Слідопити». Creature names, locations (Nimbus Fields, The Mistwoods), forms («Highland Form»), items, currencies (Glimmer), event names and terms like Bond Pact or Sparkling stay in the original English, because no Ukrainian localisation exists and players search by the English names, while elements and roles may be translated (Fire → вогонь, Support → підтримка). Several names follow the Russian client instead: **Idyll is «Ідилія»** and declined («в Ідилії»); **Prismana is «призмана»**, lowercase and declined, with «призмановий» («призманове аніімо») and «призматичний» for Prismatic; **Sparkling Form is «іскриста форма»**; **Polaris Institute is Інститут «Полярис»**, whose first word declines while the quoted name does not; **Aniilog is «Аніілог»**; **Companion Handbook is «Довідник компаньйона»**; and plain «egg» is «яйце». Rarity adjectives are translated (legendary → легендарний), but an item that carries its grade in its name keeps both in English («Legendary Aniipod: Irisalis», «Aniipod Ultra x2»), as do Bond Egg and Operation: Egg Heist. Everything the developers publish is in UTC+8, so the prompts insist on Kyiv time from the supplied notes and never leave "UTC+8" in a post. Feed titles and bodies are passed to Gemini as untrusted data with an explicit instruction not to obey anything embedded in them. User-visible UI text, reports, date labels, footer labels, and collector button labels live in `locales/uk.json`.
