...
All three drafting prompts carry the same terminology rules, so the channel reads consistently. The game is **Aniimo** and is never translated or transliterated; the creatures are «аніімо» in Ukrainian, lowercase and indeclinable («нове аніімо», «86 аніімо»); players are Pathfinders → «Слідопити». Creature names, locations (Nimbus Fields, The Mistwoods, Polaris Institute), forms («Highland Form»), items, currencies (Glimmer), event names and terms like Bond Pact or Sparkling stay in the original English, because no Ukrainian localisation exists and players search by the English names, while elements and roles may be translated (Fire → вогонь, Support → підтримка). Two names follow the Russian client instead: **Idyll is written «Ідилія»** and declined («в Ідилії»), and **Prismana is «призмана»**, lowercase and declined, with the adjective «призмановий» («призманове аніімо») and «призматичний» for Prismatic. Everything the developers publish is in UTC+8, so the prompts insist on Kyiv time from the supplied notes and never leave "UTC+8" in a post. Feed titles and bodies are passed to Gemini as untrusted data with an explicit instruction not to obey anything embedded in them. User-visible UI text, reports, date labels, footer labels, and collector button labels live in `locales/uk.json`.
